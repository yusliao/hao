...
namespace PushServer.Commands
{
    ...
    class QIYIGUOExcelOrderOption : OrderOptionBase
    {
        public override string Name => OMS.Models.OrderSource.QIYIGUO;
        ...
        protected override List<OrderEntity> FetchOrders()
        {
            ...
            foreach (var file in this.GetExcelFiles())
            {
                using (var excel = new NPOIExcel(file.FullName))
                {
                    ...
                    {
                        OnUIMessageEventHandle($"电商-奇异果电子商务导入文件：{file.FileName}解析完毕,当前订单数{ordersList.Count}");
                        continue;
                    }
        ...
        protected  List<OrderEntity> ResolveOrders(DataTable excelTable,string file, List<OrderEntity> items)
        {
            ...
            for (int i = 0; i < excelTable.Rows.Count; i++)
            {
                ...
                orderDTO.count = Convert.ToInt32(row["购买件数"]); //数量
                ...
                if (item == null)
                {
                   ...
                   // var productProps = Convert.ToString(row[5]); //商品属性
                    ...
                    orderDTO.consigneePhone = Convert.ToString(row["下单人"]); //联系电话
                  //  orderDTO.consigneePersonCard = Convert.ToString(row["身份证号"]); //联系电话
                    ...
                    orderDTO.consigneeCounty = Convert.ToString(row["区"]);
