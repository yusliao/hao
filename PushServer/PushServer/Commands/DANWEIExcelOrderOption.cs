...
namespace PushServer.Commands
{
    ...
    class DANWEIExcelOrderOption : OrderOptionBase
    {
        public override string Name => OMS.Models.OrderSource.DANWEI;
        ...
        protected override List<OrderEntity> FetchOrders()
        {
            ...
            foreach (var file in this.GetExcelFiles())
            {
                using (var excel = new NPOIExcel(file.FullName))
                {
                    ...
                    {
                        OnUIMessageEventHandle($"电商-单维导入文件：{file.FileName}解析完毕,当前订单数{ordersList.Count}");
                        continue;
                    }
