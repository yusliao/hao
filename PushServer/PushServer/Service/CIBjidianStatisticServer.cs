...
namespace PushServer.Service
{
    ...
    public class CIBjidianStatisticServer : IPandianServer
    {
        public string ServerName => OrderSource.CIBVIP;
        ...
        public  bool CreateMonthPandianReport(int monthnum)
        {
            using (var db = new OMSContext())
            {
                ...
                var lst = db.OrderProductSet.Where(s => s.MonthNum == monthnum&&s.Source==OrderSource.CIBVIP).ToList();
                var q = lst.GroupBy(p => new {p.Source, p.ProductPlatName, p.weightCode,p.weightCodeDesc, p.MonthNum });
                ...
                foreach (var item in q)
                {
                    StatisticProduct statistic = new StatisticProduct()
                    {
                        ...
                        Source = OrderSource.CIBVIP,
                        SourceDesc = Util.Helpers.Reflection.GetDescription<OrderSource>(OrderSource.CIBVIP),
                        MonthNum = monthnum,
                ...
                var removeobj = db.StatisticMonthPandianSet.Where(s => s.MonthNum == monthnum && s.Source == OrderSource.CIBVIP);
